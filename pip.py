...
def process_image(input_image_path, output_image_path, encoder, depth_decoder, device, augmentor):
    ...
    # Convert PIL image to OpenCV format for rain augmentation
    darkened_image_cv2 = np.array(image)[:, :, ::-1]  # Convert RGB to BGR for OpenCV

    # Apply rain streaks with random IDs
    rand_id1 = random.randint(1, 165)
    rand_id2 = random.randint(4, 8)
    rain_augmented_image = augmentor.apply_rain_streaks(darkened_image_cv2, rand_id1, rand_id2)
